...
class CartonSystemApp:
    ...
    def process_barcode(self, event):
        ...
        if self.awaiting_new_carton:
            ...
            with open(os.path.join(self.app_dir, 'config.ini'), 'w', encoding='utf-8') as config_file: self.config.write(config_file)
        self.last_panel_id = panel_id
        self.config.set('settings', 'last_panel_id', panel_id)
        with open(os.path.join(self.app_dir, 'config.ini'), 'w', encoding='utf-8') as config_file: self.config.write(config_file)
        self.log_message(f"讀取到 Panel ID: {panel_id}")
        if self.first_panel_id_of_carton is None: self.first_panel_id_of_carton = panel_id
        ...
        if c_serial == 1: self.reset_label_progress()
        ...
        try:
            ...
            header_columns = {str(cell.value).strip(): cell.column for cell in worksheet[1] if cell.value is not None and str(cell.value).strip()}
            ...
            if missing_headers: raise ValueError(f"Excel 缺少欄位: {', '.join(missing_headers)}")
            ...
            runtime_columns = {header: runtime_start + offset for offset, header in enumerate(runtime_headers)}
            for header, column in runtime_columns.items():
                header_cell = worksheet.cell(row=1, column=column)
                if not isinstance(header_cell, MergedCell) and header_cell.value is None: header_cell.value = header
            rows = [(row_number, worksheet[row_number]) for row_number in range(2, worksheet.max_row + 1) if str(worksheet.cell(row=row_number, column=panel_column).value or '').strip() == panel_id]
            if not rows:
                self.log_message(f"❌ Excel 中找不到該 Panel ID [{panel_id}] 的資料", "red")
                self.play_sound("buzz.wav")
                return
            successful_rows, scan_failed, skipped_rows = [], False, 0
            for row_number, row in rows:
                serials = [(str(row[s1_column - 1].value or '').strip(), "S1"), (str(row[s2_column - 1].value or '').strip(), "S2")]
            ...
            for row_number, row, serials, station_data_by_isn in successful_rows:
                ...
                for isn, side in serials:
                    ...
                    if isn not in target: target.append(isn)
                ...
                for station_name in ("SPI_T", "SPI_B", "AOI_T", "AOI_B", "AOI_B_B", "AOI_B_T"):
                    ...
                    worksheet.cell(row=row_number, column=runtime_columns[f"{station_name}_StopTime"]).value = stop_time.strftime("%Y-%m-%d %H:%M:%S") if stop_time is not None else ""
            ...
            if self.carton_scanned_count >= qty: self.trigger_print()
        except Exception as e:
            self.log_exception(e)
            messagebox.showerror("系統錯誤", f"處理過程中發生錯誤: {str(e)}")

    def save_runtime_settings(self):
        for entry_name, config_name in (("P/N:", "MySQL_PN"), ("LOT(WO#):", "MySQL_LOT"), ("Q'ty:", "MySQL_QTY"), ("MySQL_Carton:", "MySQL_Carton"), ("MySQL_Carton_Serial:", "MySQL_Carton_Serial")):
        ...
        with open(os.path.join(self.app_dir, "MySQLConfig.ini"), "w", encoding="utf-8") as config_file: self.mysql_config.write(config_file)
    ...
    def load_label_data(self):
        json_file = os.path.join(self.app_dir, self.config.get('settings', 'json_name', fallback='label_data.json'))
        if not os.path.exists(json_file): return
        try:
            with open(json_file, 'r', encoding='utf-8') as data_file: data = json.load(data_file)
            if data.get('S1QRCode1') is not None or data.get('S2QRCode1') is not None:
                self.s1_isn_list = [isn for key in ('S1QRCode1', 'S1QRCode2', 'S1QRCode3', 'S1QRCode4') for isn in str(data.get(key, '')).split(',') if isn.strip()]
                self.s2_isn_list = [isn for key in ('S2QRCode1', 'S2QRCode2', 'S2QRCode3', 'S2QRCode4') for isn in str(data.get(key, '')).split(',') if isn.strip()]
            else:
                self.s1_isn_list = [isn for key in ('QRCode1', 'QRCode5', 'QRCode3', 'QRCode4') for isn in str(data.get(key, '')).split(',') if isn.strip()]
                self.s2_isn_list = []
    ...
    def verify_database(self, isn):
        try:
            ...
            conn = pymysql.connect(host=self.mysql_config.get('setting', 'MySQL_ServerIP'), user=self.mysql_config.get('setting', 'MySQL_username'), password=self.mysql_config.get('setting', 'MySQL_Password'), database=self.mysql_config.get('setting', 'MySQL_DB'), charset='utf8', cursorclass=pymysql.cursors.DictCursor)
            cursor = conn.cursor()
            query = """SELECT c.SPI_T, c.SPI_B, c.AOI_T, c.AOI_B, c.AOI_B_B, c.AOI_B_T,
                ...
                FROM CARD c WHERE c.iSN = %s LIMIT 1"""
            cursor.execute(query, (isn,))
            result = cursor.fetchone()
            cursor.close(); conn.close()
            station_names = ("SPI_T", "SPI_B", "AOI_T", "AOI_B", "AOI_B_B", "AOI_B_T") if self.cfg_2d_aoi_flag == '1' else ("SPI_T", "SPI_B", "AOI_T", "AOI_B")
            if not result:
                self.log_message(f"❌ iSN [{isn}] 未找到 CARD 資料或尚未寫入站別資料", "red")
                return None
            normalized_codes = {station: str(result.get(f"{station}_ErrorCode") or "").strip().upper() for station in station_names}
            failed_stations = [station for station in station_names if result.get(station) is None or result.get(station) <= 100 or normalized_codes[station] not in ("PASS", "RPASS")]
            if not failed_stations: return result
            failure_summary = "全部未通過" if len(failed_stations) == len(station_names) else ", ".join(failed_stations)
            self.log_message(f"❌ iSN [{isn}] 未通過資料庫(SPI/AOI)檢驗，未通過站別: {failure_summary}", "red")
            return None
        except Exception as e:
            self.log_message(f"⚠️ 資料庫連線失敗或查詢錯誤: {e} (測試環境模擬通過)")
            return None

    def _qr_strings(self):
        has_both_sides = bool(self.s1_isn_list and self.s2_isn_list)
        if not has_both_sides:
            isn_list = self.s1_isn_list + self.s2_isn_list
            return [','.join(isn_list[i:i + 40]) for i in (0, 40, 80, 120)]
        return [','.join(self.s1_isn_list[:40]), ','.join(self.s2_isn_list[:40]), ','.join(self.s1_isn_list[40:80]), ','.join(self.s2_isn_list[40:80])]

    def write_label_json(self, pn, lot, qty):
        json_file = os.path.join(self.app_dir, self.config.get('settings', 'json_name', fallback='label_data.json'))
        qr1, qr2, qr3, qr4 = self._qr_strings()
        data = {"PN": pn, "DateCode": self.entries["D/C:"].get().strip(), "LOT WO": lot, "QTY": str(qty), "QRCode1": qr1, "QRCode5": qr2, "QRCode3": qr3, "QRCode4": qr4}
    ...
    def save_json_to_archive(self, pn, lot, qty, carton):
        ...
        data = {"PN": pn, "DateCode": self.entries["D/C:"].get().strip(), "LOT WO": lot, "QTY": str(qty), "Carton": str(carton)}
        qr1, qr2, qr3, qr4 = self._qr_strings()
        data.update({"QRCode1": qr1, "QRCode2": qr2, "QRCode3": qr3, "QRCode4": qr4})
        try:
            with open(os.path.join(json_folder, archive_filename), 'w', encoding='utf-8') as f: json.dump(data, f, ensure_ascii=False, indent=2)
            self.log_message(f"✅ JSON 備份已存至: {archive_filename}")
        except OSError as e: self.log_message(f"❌ JSON 備份失敗: {e}", "red")

    def manual_trigger_print(self):
        """手動重印目前 label；不檢查、不增加箱內計數，也不變更箱號或流水號。"""
        pn = self.entries["P/N:"].get().strip()
        lot = self.entries["LOT(WO#):"].get().strip()
        qty = self.entries["Q'ty:"].get().strip()
        carton = self.entries["MySQL_Carton:"].get().strip()
        self.save_json_to_archive(pn, lot, qty, carton)
        self.trigger_print(advance_carton=False)
    ...
    def trigger_print(self, advance_carton=True):
        """執行列印；只有掃描達量的自動列印才會換箱，手動重印不會改變計數或箱號。"""
        mode = "自動列印" if advance_carton else "手動重印"
        self.log_message(f"{mode}：開始執行 test.bat 列印程序...")
        ...
        if os.path.exists(batch_file):
            try:
                ...
                result = subprocess.run(["cmd.exe", "/c", batch_file], cwd=project_dir, timeout=120, check=False)
                output = ""
                if os.path.exists(output_file):
                    with open(output_file, "rb") as output_handle: output = output_handle.read().decode("utf-8", errors="replace")
                if result.returncode != 0 or "PASS" not in output.upper() or "[OK]" not in output.upper():
                    self.play_sound("buzz.wav")
                    self.log_message("❌ FAIL：未確認列印已成功傳送至印表機佇列", "red")
                    return
                self.log_message(f"🚀 PASS [OK]：{mode}已成功傳送至印表機佇列。")
                if not advance_carton:
                    return
            ...
            except Exception as e: self.log_message(f"❌ 列印執行失敗: {e}")
        else: self.log_message("❌ 找不到 test.bat 檔案，無法列印")
    ...
    def clear_label_data(self):
        ...
        try:
            with open(json_file, 'r', encoding='utf-8') as data_file: data = json.load(data_file)
            for key in ('QRCode1', 'QRCode5', 'QRCode3', 'QRCode4'): data[key] = ''
            with open(json_file, 'w', encoding='utf-8') as data_file: json.dump(data, data_file, ensure_ascii=False, indent=2)
        except (OSError, json.JSONDecodeError) as error: self.log_message(f"⚠️ 清除列印暫存資料失敗: {error}")
